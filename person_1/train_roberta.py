"""
Person 1 — Train RoBERTa-large
Ensemble member for AI detection. Trained on RAID + HC3 + GPT-2 Output.
"""
...
def main():
    ...
    model, tokenizer, best_metrics = train_classifier(
        ...
        checkpoint_name="roberta_ai_detector",
    )

    print(f"\n{'=' * 60}")
    print(f"  RoBERTa Training Complete")
    print(f"  Best Metrics: {best_metrics}")
    print(f"  Checkpoint:   {CHECKPOINT_DIR / 'roberta_ai_detector'}")
    print(f"{'=' * 60}")
